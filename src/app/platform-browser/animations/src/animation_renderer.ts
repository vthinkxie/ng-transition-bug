...
type NestedAnimationTriggerMetadata =
  | AnimationTriggerMetadata
  | RecursiveAnimationTriggerMetadata;
interface RecursiveAnimationTriggerMetadata
  extends Array<NestedAnimationTriggerMetadata> {}
...
export class AnimationRendererFactory implements RendererFactory2 {
  ...
  createRenderer(hostElement: any, type: RendererType2): Renderer2 {
    ...
    if (!hostElement || !type || !type.data || !type.data["animation"]) {
      let renderer: BaseAnimationRenderer | undefined = this._rendererCache.get(
        delegate
      );
      if (!renderer) {
    ...
    const animationTriggers = type.data[
      "animation"
    ] as NestedAnimationTriggerMetadata[];
...
export class AnimationRenderer extends BaseAnimationRenderer
  implements Renderer2 {
  constructor(
    public factory: AnimationRendererFactory,
    namespaceId: string,
    delegate: Renderer2,
...
function resolveElementFromTarget(
  target: "window" | "document" | "body" | any
): any {
  switch (target) {
    case "body":
